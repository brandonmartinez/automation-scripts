...
count=$(magick -ping "$infile" -format "%m\n" info: | wc -l)
...
if [ "$resize" != "" ]; then
	magick -quiet $dproc "$infile" +repage "$tmpA4" ||
		errMsg "--- FILE $infile1 DOES NOT EXIST OR IS NOT AN ORDINARY FILE, NOT READABLE OR HAS ZERO SIZE  ---"
	magick "$tmpA4" -scale $resize% "$tmpA1"
else
	magick -quiet $dproc "$infile" +repage "$tmpA1" ||
		errMsg "--- FILE $infile1 DOES NOT EXIST OR IS NOT AN ORDINARY FILE, NOT READABLE OR HAS ZERO SIZE  ---"
fi
...
# test for extend
test=$(magick xc: -format "%[fx:$extend>0?1:0]" info:)
...
if [ "$bcolor" != "" ]; then
	...
	x=$(magick xc: -format "%[fx:round($x*$resize/100)]" info:)
	y=$(magick xc: -format "%[fx:round($y*$resize/100)]" info:)
	coords="$x,$y"
	bcolor=$(magick $tmpA1 -format "%[pixel:u.p{$coords}]" info:)
elif [ "$coords1" != "" -a "$resize" = "" ]; then
	x=$(echo "$coords1" | cut -d, -f1)
	y=$(echo "$coords1" | cut -d, -f2)
	...
	widthm1=$(magick xc: -format "%[fx:$width-1]" info:)
	heightm1=$(magick xc: -format "%[fx:$height-1]" info:)
	midwidth=$(magick xc: -format "%[fx:round(($width-1))/2]" info:)
	midheight=$(magick xc: -format "%[fx:round(($height-1))/2]" info:)
...
magick $tmpA1 -fuzz ${fuzzval}% -fill none \
...
if [ "$mask" = "view" ]; then
	...
	magick $tmpA2 ${outname}_mask.gif
...
echo ""
data=$(magick $tmpA2 -precision 15 \
...
if [ "$sortval" != "" ]; then
	...
	for ((i = 0; i < num; i++)); do
		xx=${xoArr[$i]}
		yy=${yoArr[$i]}
		xr=$(magick xc: -format "%[fx:$nearest*round($xx/$nearest)]" info:)
		yr=$(magick xc: -format "%[fx:$nearest*round($yy/$nearest)]" info:)
...
for ((i = 0; i < num; i++)); do
	...
	if [ "$color" != "gray(0)" ]; then
		...
		ht=${hhArr[$i]}
		if [ "$resize" != "" ]; then
			wd=$(magick xc: -format "%[fx:round($wd*100/$resize)+2*$extend+$lt+$rt]" info:)
			ht=$(magick xc: -format "%[fx:round($ht*100/$resize)+2*$extend+$tp+$bt]" info:)
			offx=$(magick xc: -format "%[fx:round($offx*100/$resize)-$extend-$lt]" info:)
			offy=$(magick xc: -format "%[fx:round($offy*100/$resize)-$extend-$tp]" info:)
			img=$tmpA4
		else
		...
		if [ $unrotate -eq 3 ]; then
			magick $img -crop ${wd}x${ht}+${offx}+${offy} $repaging $tmpA3
		elif [ $unrotate -eq 1 ]; then
			magick $img -crop ${wd}x${ht}+${offx}+${offy} +repage $derotate \
				-bordercolor "$bcolor" -border 2 -fuzz ${fuzzval}% -trim +repage $tmpA3
		elif [ $unrotate -eq 2 ]; then
			magick $img -crop ${wd}x${ht}+${offx}+${offy} +repage \
		...
		if [ "$innertrim" = "yes" ]; then
			...
			magick $tmpA3 ${outname}-${kk}.${suffix}
		fi
		k=$((k + 1))
	fi
done
